...
| SYNC-04 | ~~Önce LAN-only mod (mDNS + QUIC), sonra relay~~ **Bitti (yerel ağ), 2026-09-25 (§11.10): `panora-sync` ikilisi, birimi ve ayrı paketi. Farklı ağlar arası senkron SYNC-07 olarak "Gelecek"e taşındı.** | P3 | L |
...
| SYNC-07 | **Gelecek (ertelendi 2026-09-25):** farklı ağlardaki cihazlar arası senkron | P3 | L |

**SYNC-07 — Gelecek.** 2026-09-25'te sahibinin kararıyla ertelendi. Şimdilik öncelik, aynı ağdaki
cihazların güvenle eşitlenmesi. `panora-sync` yalnızca loopback, RFC 1918, link-local ve IPv6 ULA
adresleriyle konuşur; tanımadığı hiçbir sunucuya bağlanmaz. Yeniden ele alındığında
değerlendirilecek yollar:
1. **VPN üzerinden:** Tailscale, WireGuard ya da ZeroTier adreslerini (ör. CGNAT `100.64.0.0/10`)
   yalnızca `sync.peers`'a elle yazıldığında kabul etmek. Sunucu yok; en küçük iş.
2. **Kendi barındırılan relay:** `panora-relay`, veriyi okumadan iletir; kullanıcı kendi
   sunucusunda çalıştırır. Relay kimin ne zaman bağlandığını görür. Kimlik anahtarı gösterilmeden
   önce gizli bir el sıkışma (ADR 0004'teki bilinen sınır) bu aşamada eklenmeli.
3. **iroh + kendi relay'i:** NAT delme hazır gelir, ama yaklaşık 208 crate / 11 MiB ekler ve
   `deny.toml` lisans sorunu var (ADR 0004).

Herkese açık bir relay işletmek, "hesap yok, sunucu yok" duruşunu değiştirdiği için ayrı bir karar
(D-9) gerektirir.
...
| **2.0.0** — "Senkron" | ADR 0002 | SYNC-01…04 (yerel ağ), SYNC-06; SYNC-05 ve SYNC-07 (farklı ağlar) sonraya | 3+ ay |
...
| D-9 | Senkron kapsamı | LAN-only önce / iroh relay / hiç | ~~LAN-only spike, 2.0~~ **Spike yapıldı (ADR 0004): LAN-only önce, quinn + mDNS; iroh +208 crate / +11 MiB ve varsayılanıyla n0.computer'a bağlanıyor. 2026-09-25: 2.0 yalnızca aynı ağda; farklı ağlar SYNC-07 ile Gelecek'e.** |
...
| Son inceleme | Birleştirmeden önce bütün yığın bir kez daha saldırgan gözüyle incelendi: gizlilik ve kimlik doğrulamayı bozan bulgu yok; 2 orta (Lamport zehirlemesiyle bir cihazın senkronunun kalıcı olarak durması, uzak zaman damgalarının saklama süresini ve sıralamayı bozması), 1 düşük–orta (PRIMARY'nin `record_primary` kapalı cihaza gelmesi) ve 5 düşük bulgu kapatıldı; ayrıntı ADR 0004 "Birleştirme öncesi son inceleme". Yeni testler: Lamport sınırları, zaman kırpma, PRIMARY, davet kanıtı (birim ve uçtan uca: 12 davetsiz istekten sonra bağlantı hâlâ çalışıyor). |
...
Açık kalanlar: SYNC-05 Android;
SYNC-06 bağımsız güvenlik incelemesi (yayın öncesi şart, o zamana kadar paket "deneysel").
Farklı ağlar arası senkron (relay/VPN) 2026-09-25'te sahibinin kararıyla ertelendi: SYNC-07, §5.11.
